...
describe('POST /users/login/ - user login', () => {
  it('user login any without params', async () => {
    ...
    expect(data.data.email).toEqual(expect.arrayContaining(['Email can\'t be blank']));
    expect(data.data.password).toEqual(expect.arrayContaining(['Password can\'t be blank']));
  });
  ...
  it('user login with invalid email', async () => {
    ...
    expect(data.data.password).toEqual(expect.arrayContaining(['Email belum terdaftar']));
  });
